...
namespace TRA_Invoicing_Console
{
    class FundCompanyInvoice
    {
        ...
        protected static string mainViewName = ConfigurationManager.AppSettings.Get("MainViewName") + "_F";
        protected static string detailViewName = ConfigurationManager.AppSettings.Get("DetailViewName") + "_F";

        protected static string outputDirectory = ConfigurationManager.AppSettings.Get("OutputDirectory");
        protected static string outputFileName = ConfigurationManager.AppSettings.Get("FundCoInvoice_OutputFileName");
      ...
      //  protected static ReportViewer reportViewer1 = new ReportViewer();
        ...
        public static string CreateInvoice()
        {
            ...
            try
            {
                                ...
                                sqlConnStringBase))
                {
                    ...
                    using (var command = new SqlCommand("CreateInvoiceInfo_F_View", sqlConnection)
                    {
                    ...
                    using (var command = new SqlCommand("CreateInvoiceDetail_F_View", sqlConnection)
                    {
                        CommandType = CommandType.StoredProcedure
                    })
                    {
                ...
                using (ReportViewer reportViewer1 = new ReportViewer())
                {
                    ...
                    reportViewer1.LocalReport.ReportEmbeddedResource = "TRA_Invoicing_Console.Invoice_Main_F.rdlc";
        ...
        static void SetSubDataSource(object sender, SubreportProcessingEventArgs e)
        {
            ...
            string fundCoName = e.Parameters["CompanyName_Parameter"].Values[0];
                            ...
                            sqlConnStringBase))
            {
                ...
                //Invoice Detail
                string selectString = "SELECT * FROM dbo." + detailViewName + " WHERE FundCompany = '" + fundCoName + "'"; // AND FundName = '" + fundName + "'";
                using (SqlDataAdapter daDetail = new SqlDataAdapter(
                                selectString, sqlConnection))
                {
                    dsInvoiceData.Clear();
